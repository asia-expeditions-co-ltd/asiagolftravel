<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;

use \App\GolfPackage;
use \App\Country;
use App\Province;
use App\Web;

use Illuminate\Support\Facades\Mail;
use \App\Mail\RequestTeeTime;
use \App\Mail\ContactUs;
use App\Tour;
use App\Category;
use App\Events;
use App\Slide;
use App\Supplier;
use App\CountView;
use Validator;

class HomeController extends Controller
{
    //redirect to the home page of webpage
    public function getHome(){

        $getSlide = Web::find(config('app.web'))->slide()->orderBy('id', 'DESC')->get();
        $webs = Web::find(config('app.web'))->tour()->where(['status'=>1, 'type'=>1])->whereIn('country_id', [30, 122,175,206])->inRandomOrder()->limit(8)->get();

        return view('index', compact('webs', 'getSlide'));
    }

    public function getDestination(Request $req){
        if ( !empty($req->location) && $req->tour_type != '' && !empty($req->cat) ) {
            $prov=Province::where('country_id', $req->location)->first();   
            $cat = Category::find($req->cat);       
            $tours=Tour::getcatTour($req->cat, config('app.web'), $req->tour_type, $prov->id);
        }elseif (!empty($req->location) && $req->tour_type != '') {
            $cat = Category::find($req->cat);
            $prov=Province::where('country_id', $req->location)->first();
            $tours=Web::find(config('app.web'))->tour()->where(['status'=>1, 'type'=> $req->tour_type, 'province_id'=>$prov->id])->orderBy('id', 'DESC')->paginate(12);
        }elseif (!empty($req->location)) {
            $prov=Province::where('country_id', $req->location)->first();     
            $tours=Web::find(config('app.web'))->tour()->where(['status'=>1, 'province_id'=>$prov->id])->orderBy('id', 'DESC')->paginate(12);
            $cat = Category::find($req->cat);
        }elseif ($req->cat != '') {
            $prov= '';
            $cat = Category::find($req->cat);
            $tours=Tour::getcatTour($req->cat, config('app.web'));
        }else{
            $cat = Category::find(0);
            $tours=Web::find(config('app.web'))->tour()->where(['status'=>1,'type'=> 1])->orderBy('id', 'DESC')->paginate(12);
        }
        // return $cat;
        $recentTour=Web::find(config('app.web'))->tour()->where(['status'=>1, 'type'=> 1])->take(6)->orderBy('id', 'DESC')->get();

        $prov = Province::where(['country_id'=> $req->location, 'province_order'=> 1])->get();
        $country = Country::where(['id'=> $req->location])->first();
        $tourgolf = Tour::where(['country_id'=> $req->location,'status'=>1, 'type'=>1])->get();
        $des_sup =Supplier::where(['country_id'=> $req->location,'status'=>1])->get();
        
        return view('destination.countrygolf', compact('des_sup', 'tours', 'prov', 'recentTour', 'cat','country','tourgolf'));
        
    }

    // get view single tour
    public function getTour($dest_Name){
  
        $tour = Web::find(config('app.web'))->tour()->where(['status'=>1,'slug'=>$dest_Name])->orderBy('id', 'DESC')->first();

          $dataip    = unserialize(file_get_contents('http://www.geoplugin.net/php.gp?ip='.$_SERVER['REMOTE_ADDR']));
    $data      = $dataip['geoplugin_request'];
    $today     = date('Y-m-d 00:00:00');

         // $yesterday = date('Y-m-d 00:00:00',strtotime(date($dt). "+1 days"));


    // if (\Auth::user()->role_id==1) {
    //    return view('include.packagesdetails', compact('tour'));

    // }
    // else{
      if ( !CountView::Getdate($today, $data , $tour->id)) {
                $adds                 = new CountView;
                $adds->ip             = $dataip['geoplugin_request'];
                $adds->cityName       = $dataip['geoplugin_city'];
                $adds->countryName    = $dataip['geoplugin_countryName'];
                $adds->timezone       = $dataip['geoplugin_timezone'];
                $adds->tour_id        = $tour->id;
                $adds->web_id         = 2;
                $adds->save();   
            }
          // }
        return view('include.packagesdetails', compact('tour'));
    
  }




    // public function getGolfPackage(Request $req) {
    //         $get_con = Country::where('country_slug', $req->by)->first();
    //  if (isset($req->by)) {            
    //      $golfpackage = GolfPackage::where(['country_id'=>$get_con->id, 'type'=> 2])->orderByRaw("RAND()")->take(6)->get();
    //         $golfCourse = GolfPackage::where(['country_id'=>$get_con->id, 'type'=> 1])->orderByRaw("RAND()")->take(6)->get();
    //      $active = $get_con->id;
    //  }else{
    //      $active = '';
    //      $golfpackage = GolfPackage::where('type', 2)->orderBy('id', 'DESC')->orderByRaw("RAND()")->take(6)->get();
    //         $golfCourse = GolfPackage::where('type', 1)->orderBy('id', 'DESC')->orderByRaw("RAND()")->take(6)->get();
    //  }
    //     // return $get_con;
    //  return view('destination.destination', compact('golfpackage', 'golfCourse', 'get_con','active'));
    // }

    // public function getGolfCourse($country){
    //     $get_con = Country::where('country_slug', $country)->first();
    //     $golfpackage = GolfPackage::where(['country_id'=>$get_con->id, 'type'=> 2])->orderByRaw("RAND()")->take(6)->get();
    //     $golfCourse = GolfPackage::where(['country_id'=>$get_con->id, 'type'=> 1])->orderByRaw("RAND()")->take(6)->get();
    //     return view('destination.destination', compact('golfpackage', 'golfCourse', 'get_con'));
    // }

    // public function getGoflDetails($country, $province, $golfName){
    //     $golf = GolfPackage::where('golf_slug', $golfName)->first();
    //     return view("destination.singleView", compact('golf'));
    // }

    // // activities section making
    public function getActivity(){
        $ourNews = Events::orderBy('id', 'DESC')->paginate();
        return view('activity.activity', compact('ourNews'));
    }

    // view single view openssl_pkey_get_details(key)
    public function singActivity($New){
        $Onew = Events::where('slug', $New)->first();
        return view('activity.singActivity', compact('Onew'));
    }

    // // for request tee time()
    // public function getRequestTeeTime(Request $req){
    //     $data = ['date' => $req->date,
    //             'teetime' => $req->teetime,
    //             'phone' => $req->phone,
    //             'email' => $req->email, 
    //             'player' => $req->player,
    //             'link' => $req->link,
    //             'message' => $req->message ];
    //     Mail::to(config('app.email'))->send(new RequestTeeTime($data));
    //     return back()->with('message' , 'Tees Time Request Successfully');
    // }

    // // for contact us anytime
    public function getContactUs(){
        return view('contact.contactUs');
    }

    // for contact sent message
    
    public function sendContact(Request $req){

        $data = ['date'=>$req->date,
                 'pax'=>$req->pax_number,
                 'url' => $req->url, 
                 'title'=> $req->title,
                 'phone'=> $req->phone, 
                 'email'=> $req->email, 
                 'message'=> $req->message,
             ];

               // return config('app.email');
        Mail::to(config('app.email'))->send(new ContactUs($data));
        return back()->with(['message' => 'Your request booking has been submitted we will contact you back in shortly..! Thanks you.']); 
         }

    public function gettour_pack(){
         $data_cunt = Country::getgolf_t();
            foreach ($data_cunt as  $key => $value) {
          $value->c_id; 
        }
         $data_tour = Tour::where(['country_id'=>$value->c_id, 'type'=>1])->get();
         return view('golf_packages.tours_package', compact('data_tour', 'data_cunt'));
    }
    public function tour_pack($data_id){
         $data_cont = Web::find(config('app.web'))->tour()->where(['country_id'=>$data_id, 'type'=>1,'status'=>1])->get();
     foreach ($data_cont as $value) {
       $data=array( 'pro_name'   => isset($value->province->province_name)? $value->province->province_name: '',
                    'photo'    => $value->photo,
                    'title'    => $value->title,
                    'country_id'=>$value->country_id,
                    'type'      =>$value->type,
                    'slug'     => $value->slug);
       $mydata[]=$data;}
       return response()->json(['mytour'=>$mydata]); 
    }

    // ===golf_courses==========>
    public function getgolf_courses(){
          $data_cunt = Country::getgolf_c();
        foreach ($data_cunt as  $key => $value) {
          $value->c_id; 
        }
        $data_tour = Tour::where(['country_id'=>$value->c_id, 'type'=>2])->get();
        return view('golf_hotels.golf_courses', compact('data_cunt','data_tour'));
    }
    public function golf_courses($data_id){
       $data_cont = Web::find(config('app.web'))->tour()->where(['country_id'=>$data_id, 'type'=>2,'status'=>1])->get();
     foreach ($data_cont as $value) {
       $data=array( 'pro_name'   => isset($value->province->province_name)? $value->province->province_name: '',
                    'photo'    => $value->photo,
                    'title'    => $value->title,
                    'country_id'=>$value->country_id,
                    'type'      =>$value->type,
                    'slug'     => $value->slug);
       $mydata[]=$data;}
       return response()->json(['data_golf'=>$mydata]); 
    }
    // ===end golf_courses=======<

    public function gethotel_resorts(){
          $data_cunt = Country::getgolf_r();
                 foreach ($data_cunt as  $key => $value) {
          $value->c_id; 
        }
         $data_cont = Tour::where(['country_id'=>$value->c_id, 'type'=>3])->get();
         return view('golf_hotels.hotels_resorts', compact('data_cunt','data_cont'));
    }

    public function hotel_destination($h_id){
     $data_cont = Web::find(config('app.web'))->tour()->where(['country_id'=>$h_id, 'type'=>3,'status'=>1])->get();
     foreach ($data_cont as $value) {

       $data=array('pro_name'   => isset($value->province->province_name)? $value->province->province_name: '',
                    'photo'     => $value->photo,
                    'title'     =>$value->title,
                    'country_id'=>$value->country_id,
                    'type'      =>$value->type,
                    'slug'=>$value->slug);
       $mydata[]=$data;
     }
       return response()->json(['data'=>$mydata]); 
    }

    public function golfDetails($dest_Name){

         $tour = Web::find(config('app.web'))->tour()->where(['status'=>1,'slug'=>$dest_Name])->orderBy('id', 'DESC')->first();

                 $dataip    = unserialize(file_get_contents('http://www.geoplugin.net/php.gp?ip='.$_SERVER['REMOTE_ADDR']));
        $data      = $dataip['geoplugin_request'];
        $today     = date('Y-m-d 00:00:00');

             // $yesterday = date('Y-m-d 00:00:00',strtotime(date($dt). "+1 days"));
          if ( !CountView::Getdate($today, $data , $tour->id)) {
                    $adds                 = new CountView;
                    $adds->ip             = $dataip['geoplugin_request'];
                    $adds->cityName       = $dataip['geoplugin_city'];
                    $adds->countryName    = $dataip['geoplugin_countryName'];
                    $adds->timezone       = $dataip['geoplugin_timezone'];
                    $adds->tour_id        = $tour->id;
                    $adds->web_id         = 2;
      
                    $adds->save();   
                }
         // return $tour;
            return view('golf_hotels.golf_courses_details', compact('tour'));


    }


     public function getsupdes(Request $re){
            $suppitetm = Supplier::where(['status'=>1,'id'=>$re->id])->orderBy('id', 'DESC')->get();
            $mytour = Web::find(config('app.web'))->tour()->where(['status'=>1,'supplier_id'=>$re->id])->orderBy('id', 'DESC')->get();
            return view('destination.supplier_item', compact('suppitetm', 'mytour'));
        }

      public function getsupdetail(Request $re){
        $tour = Web::find(config('app.web'))->tour()->where(['status'=>1, 'tbl_tours.id'=>$re->id])->orderBy('id', 'DESC')->first();
            return view('destination.packagesdetails', compact('tour'));
        }

       public function getprovince(Request $re){

          $data = Province::where(['id'=>$re->id])->first();
            $mytour = Web::find(config('app.web'))->tour()->where(['status'=>1,'province_id'=>$re->id])->orderBy('id', 'DESC')->get();
            // return $data;
          return view('destination.province_detail', compact('data', 'mytour')); 
        
       }


       public function all_detail($data,$data_type){
        $get_c=Tour::where(['country_id'=>$data,'type'=>$data_type])->first();
    $data_all = Web::find(config('app.web'))->tour()->where(['country_id'=>$data,'type'=>$data_type,'status'=>1])->orderBy('id', 'DESC')->paginate(6);
    return view('golf_hotels.all_detail', compact('data_all','get_c')); 

       }

       public function getmysearch(Request $request){
   
    $output="";
      $products = Tour::where([['title','LIKE','%'.$request->search."%"],['status', 1]])->get(); 
        if($products){
          foreach ($products as  $product) {
            $output.='<a  class="addemail" data-id="'.$product->title.'"><input type="hidden" >'.$product->title.'</a>';
          }
        }
          // return $products;
          return Response($output); 
         
  }

   public function showsearch(Request $request){
 
   
     $getdata = Tour::where([['title','LIKE','%'.$request->search."%"],['status', 1]])->paginate(8);

return view('search.search', compact('getdata'));
        }

   

       
    }
