    ...
    public function createCountry(Request $req){
    	 try {
          ...
          if ( $req->hasFile('picture') ) {
              ...
              $filenamep  = time().'-'.$picture->getClientOriginalName();
              $img        = Image::make($picture->getRealPath())->fit(300, 300);
              $img->save(public_path('photos/share/thumbs/'.$filenamep));
            $picture->move(public_path('photos/share/'), $filenamep);
          }else{
              $filenamep = "";
          }
	        if ( $req->hasFile('flag') ) {
          ...
          $count->country_picture = $filenamep;
	        $count->flag            = $flag;
	        $count->save();
    ...
    public function updateCountry(Request $req){
    	try {
          ...
          if ( $req->hasFile('picture') ) {
              $picture    = $req->file('picture');
              $filenamep  = time().'-'.$picture->getClientOriginalName();
              $img        = Image::make($picture->getRealPath())->fit(400, 270);;
              $img->save(public_path('photos/share/thumbs/'.$filenamep));
            $picture->move(public_path('photos/share/'), $filenamep);
          }else{
               $filenamep = $req->old_imagep;
          }
	        if ( $req->hasFile('flag') ) {
